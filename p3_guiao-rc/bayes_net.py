...
class BayesNet:
    ...
    def gen_conjunctions(self, var_list):
        if len(var_list) == 1:
            return [[(var_list[0], True)], [(var_list[0], False)]]

        remain = self.gen_conjunctions(var_list[1:])

        o = []
        for var in remain:
            o.append(var + [(var_list[0], True)])
            o.append(var + [(var_list[0], False)])
        return o
